...
function formatDateRange(from: string, to: string): string {
  ...
  const dayStart = DAY_LABELS[DAY_KEYS[start.getDay() === 0 ? 6 : start.getDay() - 1]] ?? "";
  const dayEnd = DAY_LABELS[DAY_KEYS[end.getDay() === 0 ? 6 : end.getDay() - 1]] ?? "";
  const sameYear = start.getFullYear() === end.getFullYear();
  return sameYear
    ? `${dayStart} ${start.getDate()} ${MONTH_SHORT[start.getMonth()]} – ${dayEnd} ${end.getDate()} ${MONTH_SHORT[end.getMonth()]} ${end.getFullYear()}`
    : `${dayStart} ${start.getDate()} ${MONTH_SHORT[start.getMonth()]} ${start.getFullYear()} – ${dayEnd} ${end.getDate()} ${MONTH_SHORT[end.getMonth()]} ${end.getFullYear()}`;
}
...
function buildDayMap(data: DashboardWeekResponse): Map<
  string,
  {
    dateStr: string;
    dayLabel: string;
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    return () => ac.abort();
  }, [token, year, week, logout, router]);

  const dayMap = data ? buildDayMap(data) : null;
  ...
  if (loading && !data) {
    return (
      <div className="flex justify-center py-12">
        <p className="text-sm text-[var(--app-fg)]/60">Cargando semana…</p>
      </div>
    );
  }
